...
public class TitleScreen2 extends Activity {
	private int first = -1;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_title_screen2);
		Bitmap source = BitmapFactory.decodeResource(this.getResources(), R.drawable.patday1);
		...
		btnCheck.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View view){
				if (tiles[0].sameAs(randomTiles[0]) == true && tiles[1].sameAs(randomTiles[1]) == true && tiles[2].sameAs(randomTiles[2]) == true && tiles[3].sameAs(randomTiles[3]) == true && tiles[4].sameAs(randomTiles[4]) == true && tiles[5].sameAs(randomTiles[5]) == true && tiles[6].sameAs(randomTiles[6]) == true && tiles[7].sameAs(randomTiles[7]) == true && tiles[8].sameAs(randomTiles[8])) {
					AlertDialog.Builder builder = new AlertDialog.Builder(TitleScreen2.this);
					builder.setMessage("Good Job! You unlocked Once Upon A Time")
					       .setCancelable(false)
					       .setPositiveButton("OK", new DialogInterface.OnClickListener() {
					           public void onClick(DialogInterface dialog, int id) {
					        	   Intent intent = new Intent(getApplicationContext(), PatrickActivity.class);
